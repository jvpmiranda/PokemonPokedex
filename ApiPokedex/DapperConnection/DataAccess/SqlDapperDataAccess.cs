...
public class SqlDapperDataAccess : ISqlDapperDataAccess
{
    ...
    public async Task<IEnumerable<V>> ExecuteQueryStoredProcedure<T, U, V, W>(string command, Func<T, U, V> map, W parameters, string splitOn = "")
    {
        using IDbConnection conn = new SqlConnection(_connectionString);
        return await conn.QueryAsync(command, map, parameters, commandType: CommandType.StoredProcedure, splitOn: splitOn);
    }

    public async Task<IEnumerable<W>> ExecuteQueryStoredProcedure<T, U, V, W, X>(string command, Func<T, U, V, W> map, X parameters, string splitOn = "")
    {
        using IDbConnection conn = new SqlConnection(_connectionString);
        return await conn.QueryAsync(command, map, parameters, commandType: CommandType.StoredProcedure, splitOn: splitOn);
    }
